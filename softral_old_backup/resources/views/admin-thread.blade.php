...
<div class="row content" >	
	...
	<a href="{{url('admin-manage-setting')}}" class="btn btn-danger btn-sh pull-right" style="margin-top:8px">Softral Ticket Home</a><br/><br/>
					@if(Session::has('succ'))
		<br/>
		<div class="row">
			<div class="col-lg-12">
		...
		<br/>
			...
			<div class="alert bg-danger" role="alert">
					<svg class="glyph stroked cancel"><use xlink:href="#stroked-cancel"></use></svg><font size="4"> {{ Session::get('error') }}</font>
				</div>
...
@foreach($errors->all() as $error)
	...
	@endforeach
				@endif	
				<div class="panel-heading" id="accordion" ><svg class="glyph stroked two-messages"><use xlink:href="#stroked-two-messages"></use></svg> 
...
<table class="panel-heading" style="margin-top:-50px;width:100%" >
	<tr>
		<td>
			...
			<td>	
				Status: <select id="tstatus" name="tstatus">
										<option @if($data->tstatus=="Open"){{"selected"}} @endif>Open</option>
										<option @if($data->tstatus=="Resolved"){{"selected"}} @endif>Resolved</option>
									</select>
				...
				<form class="form-horizontal" action="{{route('sendAdmin',$data->id)}}" method="POST">
				<input type="hidden" name="_token" value="{{csrf_token()}}">
