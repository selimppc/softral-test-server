...
//                            where a.sender_id in('" . $_SESSION['id'] . "', '$sendto') and a.reciever_id in('" . $_SESSION['id'] . "', '$sendto')");
    ...
    $sql = $dbh->prepare("SELECT * FROM (
                            ...
                            inner join users as b on a.sender_id = b.id 
                            inner join user_profile as c on b.id = c.user_id 
                            where a.sender_id in('" . $_SESSION['id'] . "', '$sendto') and a.reciever_id in('" . $_SESSION['id'] . "', '$sendto') ORDER BY id DESC LIMIT 100) sub ORDER BY id ASC");
    $sql->execute();
    ...
    while ($r = $sql->fetch()) {
        ...
        if ($msg_type == 'file'){
            ...
            $msg = "<a href='{$url}uploads/files/" . $r['msg'] . "' download> <img src='{$url}images/download.png' width='25px' height='25px'></a>";
        }
